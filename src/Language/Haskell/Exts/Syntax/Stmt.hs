...
module Language.Haskell.Exts.Syntax.Stmt where
...
data Stmt___ pat exp decl l
    ...
    | Qualifier l (exp l)   -- ^ an /exp/ by itself: in a @do@-expression,
                            ...
                            --   a guard expression
    | LetStmt l (Binds__ exp decl l)   -- ^ local bindings
    | RecStmt l [Stmt___ pat exp decl l]    -- ^ a recursive binding group for arrows
...
data QualStmt___ pat exp decl l
    = QualStmt     l (Stmt___ pat exp decl l)         -- ^ an ordinary statement
